	...
	public function viewClassVideos($request)
	{
        ...
        if(!empty($dats))
        {
			foreach ($dats as $r)
            {
				...
				if($r->status==1)
					  $btn.='<a href="'.url('deactivate_class_video').'/'.$r->id.'" class="btn bt-warning btn-secondary btn-elevate btn-circle btn-icon" title="Deactivate"><i class="fa fa-times"></i></a>'; 	
				else
					 $btn.='<a href="'.url('activate_class_video').'/'.$r->id.'" class="btn bt-success btn-secondary btn-elevate btn-circle btn-icon" title="Activate"><i class="fa fa-check"></i></a>'; 	
				$uData['action'] = $btn;
