...
{        
	...
	if(!empty($payment) and !empty($payment_id)) 
	{
		try 
		{
...
/*public function set_payment(Request $request)  //package purchase payment
{
...
public function get_user_courses_post()
{
	$stid=$this->post('student_id');
	$crsdt=$this->Model_user->get_user_courses($stid);
	if(!empty($crsdt))
	{
		$user['data']= $crsdt;
