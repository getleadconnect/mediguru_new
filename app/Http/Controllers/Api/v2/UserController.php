...
use App\Models\Admin; //for chat app
...
class UserController extends Controller
{
	...
	public function login(Request $request) 
	{
		...
		if (!empty($user)) 
		{
			//update fcm token ------------------------------
			$uid=$user->id;
			$fcmT=['fcm_token'=>$request->fcm_token];
			$updt=User::where('id',$uid)->update($fcmT);
			//-----------------------------------------------
...
public function admin_user_login(Request $request) 
	{
		...
		$validator = Validator::make($request->all(), [
		  'email' => 'required|string',   //mobile no
		  'password' => 'required|string',
		]);
		...
		$auser = Admin::where($where)->first();
		
		$email=$request->email;   //mobile no
					
		if (!empty($auser)) 
		{
				if($auser->status==1)
				{
					if(Hash::check($request->password, $auser->password)) 
					{
						Auth::login( $auser );
						$token = $auser->createToken('becompetitive')->accessToken;
