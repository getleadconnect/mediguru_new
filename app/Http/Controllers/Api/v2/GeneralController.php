   ...
   public function get_courses(Request $request) 
	{
		...
		$crs = Course::select('courses.*')->where('course_type',0)->get()->toArray();
		$crs1=Course::select('courses.*')->whereIn('unique_id',UserHiddenCourse::select(['course_unique_id'])->where('student_id',$stid))->get()->toArray();
				
		if(!empty($crs1))
		{
			$crs = array_merge($crs,$crs1);
		}
				
		//$crs=$crs->sortBy('reorder_no');
		
		array_multisort(array_column($crs, 'reorder_no'), SORT_ASC, $crs);
