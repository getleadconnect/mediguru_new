...
<form method="post" action="{{ url('update_subject_package')}}" enctype="multipart/form-data">
		@csrf
	   ...
	   <div class="form-group">
		...
		<input type="hidden" name="ed_course_id" value="{{$pkg->course_unique_id}}">
		...
		<label>Subject Name </label>
			...
			<option value="">--select--</option>
			@foreach($sub as $r)
			<option value="{{$r->id}}" @if($r->id==$pkg->subject_id){!! "selected" !!} @endif>{{$r->subject_name}}</option>
			@endforeach
			</select>
		</div>
		@endif
		...
		<label>Description </label>
			<textarea name="description" id="ed_description" class="textarea1 form-control" required>{!! $r->description !!}</textarea>
		</div>
