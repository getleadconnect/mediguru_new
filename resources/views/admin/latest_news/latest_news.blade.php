						...
						<div class="card-body" style="background:#e2f0f7;">
							...
							<div class="col-lg-3 col-xl-3 col-xxl-3">
								<label>News Icon (.jpg|.png only) </label>
									<input type="file" onchange="fileValidation();" id="news_icon" class="form-control" name="news_icon" required>
							</div>
							...
							<div class="col-lg-4 col-xl-4 col-xxl-4">
 							  <label>News/Live Class Title </label>
								  <input type="text" id="news_title" class="form-control" name="news_title" required>
						  	</div>
							...
							<div class="col-lg-3 col-xl-3 col-xxl-3">
								...
								<select id="news_type" class="form-control" name="news_type" required>
								  <option value="">--select--</option>
								  <option value="1">Live Class (Event)</option>
							...
							<div class="col-lg-6 col-xl-6 col-xxl-6">
								<label>Live class Link (Youtube)</label>
									<input type="text" id="event_link" class="form-control" name="event_link">
							</div>
							...
							<div class="form-group mb-0">
								<label id="ntitle">Description </label>
									<textarea rows=3 class="textarea1 form-control content1" name="description" style="min-height:100px !important;" required></textarea>
								</div>
...
$('#btnAdd').click(function()
{
   ...
   $("#news_icon").change(function() {
        var file = document.getElementById("news_icon").files[0];
        if (file) {
...
$("#news_type").change(function()
{
	...
	if(ntype==2) /*Other option*/
	{
		...
		$("#event_link").prop('disabled',true);
		$("#event_link").prop('required',false);
		$("#chat_link").prop('disabled',true);
		$("#chat_link").prop('required',false);
		$("#class_date").prop('disabled',true);
		$("#class_date").prop('required',false);
		$("#ntitle").html('Enter new details');
	}
	else
	{
		$("#event_link").prop('disabled',false);
		$("#event_link").prop('required',true);
		$("#chat_link").prop('disabled',false);
		$("#chat_link").prop('required',true);
		$("#class_date").prop('disabled',false);
		$("#class_date").prop('required',true);
		$("#ntitle").html('Description');
	}
 ...
 $('#datatable tbody').on( 'click', '.edit', function ()
  ...
  $('#datatable tbody').on( 'click', '.desc_more', function ()
  {
	var id=$(this).attr('id');
  ...
  $('#datatable tbody').on( 'click', '.act_deact', function (e)
   {
	   ...
	   var op=$(this).attr('rel');
		...
		if(confirm("Are you sure, Activate/Deactivate this details?"))
		{
				jQuery.ajax({
				type: "GET",
				url: "activate_deactivate_news"+"/"+op+"/"+id,
				dataType: 'html',
				//data: {vid: vid},
				success: function(res)
				{
